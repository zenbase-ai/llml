...
export async function orchestrateDeploymentWorkflow(pipelineId: string, runId: string, environment: string) {
  ...
  const decision = await callAI(workflowPrompt, {
    ...
    temperature: 0.2,
    responseFormat: "json",
    systemPrompt: "You are an expert DevOps engineer specializing in CI/CD pipeline management and risk assessment",
  })
  ...
  async getState({ runId }: { runId: string }) {
    return {
      ...
      previousRuns: [
        ...
        { id: "run-122", status: "rollback", duration: "18m", reason: "high error rate" },
      ],
    }
  },
}
...
async function callAI(
  ...
  return {
    ...
    monitoring: {
      metrics: ["error_rate", "latency_p95", "throughput", "external_service_health"],
      alertThresholds: {
        error_rate: 0.005,
        latency_p95: 400,
        throughput: 1000,
