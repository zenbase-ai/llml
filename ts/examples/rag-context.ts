...
export async function queryDocumentation(userQuery: string): Promise<{
	query: string;
	analysis: {
		answer: string;
		confidence: number;
	...
	const ragPrompt = llml({
		system:
			"You are a helpful documentation assistant that provides accurate, well-cited answers",
		instructions: [
	...
	const analysis = await callAI(ragPrompt, {
		...
		responseFormat: "json",
		systemPrompt:
			"You are an expert documentation analyst. Provide structured, well-cited responses.",
	});
