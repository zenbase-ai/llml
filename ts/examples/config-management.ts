...
export async function validateEnvironmentConfig(environment: string, applicationName: string) {
  // 1. Fetch data from external services
  const envConfig = await configService.getEnvConfig({ env: environment })
  const secrets = await secretsManager.getSecrets({ _namespace: applicationName })
  const compliance = await complianceService.getRequirements({ env: environment })
    ...
    configuration: {
      infrastructure: {
        ...
        database: {
          ...
          connectionSecure: envConfig.database.ssl ? "SSL enabled" : "SSL disabled",
        },
      ...
      security: {
        ...
        secretsHealth: secrets.map(s => ({
          ...
          daysSinceRotation: Math.floor((Date.now() - new Date(s.lastRotated).getTime()) / (1000 * 60 * 60 * 24)),
...
const configService = {
  async getEnvConfig({ env }: { env: string }) {
    return {
      ...
      database: {
        host: env === "production" ? "prod-db-cluster.internal" : "dev-db.internal",
        port: 5432,
