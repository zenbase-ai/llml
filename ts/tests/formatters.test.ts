...
describe("Custom Formatters", () => {
  ...
  describe("Basic Formatter Functionality", () => {
    it("should apply custom formatter to matching values", () => {
      ...
      const result = llml({ user: new MyDomainType("alice"), count: 42 }, formatterSet)
  ...
  describe("Complex Data Structures", () => {
    it("should apply formatters to values within arrays", () => {
      ...
      const formatterSet = formatters.vibeXML({
        formatters: [
          [
            (v: unknown): boolean => v instanceof Money,
            (v: unknown) => `${(v as Money).amount} ${(v as Money).currency}`,
          ],
        ],
      })
      const result = llml(
